...
set fptuser=tomcat8080
set ftppassword=tomcat8080

rem 邮件设置
set from=zhangqi@dbgo.cn
set user=zhangqi@dbgo.cn
set pass=aaaaaaaa
set to=zhangqi@dbgo.cn
rem ,qufengle@dbgo.cn,wangfengbo@dbgo.cn,liubinghong@dbgo.cn,xuwei@dbgo.cn,zhuyalin@dbgo.cn
...
svn update %WORKING_COPY% >svn_log.txt
...
echo %date% %time% PSexeC 停止tomcat  >> %script_bat%\qiweb_auto_log.txt
%script_bat%\PsExec.exe \\192.168.0.91 -u administrator -p P@ssword D:\idea+maven+jdk+tomcat\apache-tomcat-8.0.29\bin\shutdown.bat

echo %date% %time% PSexeC 删除tomcat之前的项目webapp下acme文件夹  >> %script_bat%\qiweb_auto_log.txt
%script_bat%\PsExec.exe \\192.168.0.91 -u administrator -p P@ssword D:\idea+maven+jdk+tomcat\apache-tomcat-8.0.29\bin\delete_webapp.bat
...
echo @echo on
rem -----------脚本结束---------------
echo 发送钉钉通知 >>  %script_bat%\qiweb_auto_log.txt
%script_bat%\dingding.py

echo %date% %time% PSexeC 远程启动tomcat  >> %script_bat%\qiweb_auto_log.txt
%script_bat%\PsExec.exe \\192.168.0.91 -u administrator -p P@ssword D:\idea+maven+jdk+tomcat\apache-tomcat-8.0.29\bin\startup.bat
